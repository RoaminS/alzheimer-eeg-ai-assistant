# alz_ukfgpu.py – Modèle IA simplifié de détection Alzheimer via EEG simulé
...
def generate_sigma_points_tf(x, P, alpha=0.3, beta=2.0, kappa=1.0):
    """
    Génération des sigma points (Merwe) en TensorFlow, x.shape=(n,), P.shape=(n,n).
    Retourne (2n+1, n).
    """
    n = tf.shape(x)[0]
    ...
    # Factorisation de P*c
    # Pour robustesse : on peut utiliser tf.linalg.cholesky, en s'assurant que P*c est SPD
    # Sinon tf.linalg.sqrtm si P n'est pas SPD. On va supposer P reste SPD.
    sqrt_ = tf.linalg.cholesky(P * c)

    # x.shape=(n,). On veut (2n+1, n)
    x_expanded = tf.expand_dims(x, axis=0)  # (1,n)
    sigma_points = [x_expanded]  # liste de tf.Tensor
    
    for i in tf.range(n):
        col_plus = x_expanded + tf.expand_dims(sqrt_[:, i], axis=0)
        col_minus = x_expanded - tf.expand_dims(sqrt_[:, i], axis=0)
        sigma_points.append(col_plus)
        sigma_points.append(col_minus)

    return tf.concat(sigma_points, axis=0)  # (2n+1, n)
...
def unscented_transform_tf(Xsigma, Wm, Wc, noise_cov=None):
    ...
    # Chaque diff[i] -> (n,1)
    # Cov = sum_i( Wc[i] * diff[i] * diff[i].T ) + noise_cov
    # On fait un batched matmul:
    # On reshape diff -> (2n+1, n, 1)
    diff_expanded = tf.expand_dims(diff, axis=2)  # (2n+1, n, 1)
    # On veut sum sur i de Wc[i] * (diff[i] * diff[i]^T)
    # => (2n+1, n, n) -> reduce_sum => (n,n)
...
def predict_tf(x, P, Q, dt, fx, alpha=0.3, beta=2.0, kappa=1.0):
    ...
    - fx : fonction de transition (physique_eeg_model_tf)
    ...
    x_pred, P_pred = unscented_transform_tf(Xsigma_pred, *merwe_params_tf(tf.shape(x)[0], alpha, beta, kappa)[1:], noise_cov=Q)
...
def update_tf(x_pred, P_pred, z, R, hx, alpha=0.3, beta=2.0, kappa=1.0):
    ...
    # Moy, Cov sur Z
    z_mean, Pz = unscented_transform_tf(Zsigma_pred, *merwe_params_tf(tf.shape(z)[0], alpha, beta, kappa)[1:], noise_cov=R)
    ...
    Wc = merwe_params_tf(tf.shape(z)[0], alpha, beta, kappa)[2]
    Wc_reshaped = tf.reshape(Wc, (-1,1,1))  # (2n+1,1,1)
...
def physical_eeg_model_tf(x, dt, freqs=None, coupling=None, band_powers=None):
    ...
    x.shape=(n_channels,)
    """
    # On suppose freq, coupling, band_powers => tf.Tensor
    n_channels = tf.shape(x)[0]
    dx = tf.zeros_like(x)

    # Parcours en tf.map_fn
    def body(i, dx_acc):
        ...
        new_val = self_term + coupling_term
        dx_acc = tf.tensor_scatter_nd_add(dx_acc, [[i]], [new_val])
        return i+1, dx_acc

    i0 = tf.constant(0)
    cond = lambda i, dx_acc: tf.less(i, n_channels)
    _, dx_out = tf.while_loop(cond, body, [i0, dx])
...
def ukf_physical_adaptive_tf(eeg_data, fs=500, process_noise_init=0.1, measurement_noise_init=0.1):
    ...
    La partie adaptative (calculs de Q, R, etc.) reste en Python, mais
    les étapes UKF (predict/update) sont toutes en TF.
    """
    ...
    for t in range(1, n_samples):
        ...
        if window_data.shape[1] >= 10:
            # On fait l'adapt en numpy (spectres, corr), puis on convertit en tf
        ...
        def fx_local(state, dt_local):
            return physical_eeg_model_tf(state, dt_local,
        ...
        # On stocke dans out, on réinjecte dans x_tf, P_tf
        out[:, t] = x_upd_np
        x_tf = tf.constant(x_upd_np, dtype=tf.float32)
        P_tf = P_upd  # reste en tf

        # On sauvegarde ces variables (pour la prochaine itération)
...
# >>> Traitement patient par patient (A ou AD), parallélisation interne sur les segments <<<
    ...
    if sub_dir.startswith("sub-"):
        ...
        if os.path.exists(eeg_path):
            try:
                ...
                # Filtrage UKF en parallèle (multi-thread) : on utilise notre version TF
...
# Simul. Alzheimer (3 stades : 1,2,3)
...
#
# Ce script "alz_ukfgpu.py" fait partie du projet Alzheimer EEG AI Assistant,
