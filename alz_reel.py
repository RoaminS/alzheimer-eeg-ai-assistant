...
def sanitize_eeg(eeg_data, verbose=False):
    ...
    return eeg_data, too_flat  # Retourne aussi le masque des canaux plats
...
def ukf_physical_adaptive(eeg_data, fs=500, process_noise_init=0.1, measurement_noise_init=0.1):
    ...
    for t in range(1, n_samples):
        ...
        if np.any(np.isnan(ukf.x)) or np.any(np.isinf(ukf.x)):
            print(f"⚠️ NaN/Inf dans ukf.x au timestep {t}")
            break
        out[:, t] = ukf.x
...
def normalize_eeg(segment):
    ...
    if np.any(np.isnan(normalized)) or np.any(np.isinf(normalized)):
        print("⚠️ NaN/Inf après normalisation, remplacement par 0")
...
flat_channels_counter = np.zeros(num_electrodes)  # Compteur des canaux plats
    ...
    if sub_dir.startswith("sub-"):
        ...
        if os.path.exists(eeg_path):
            try:
                ...
                flat_channels_counter += flat_mask.astype(int)  # Ajouter les canaux plats détectés
                total_samples = data_eeg.shape[1]
                num_segments = total_samples // samples

                patient_id = sub_dir
                participant = participants[participants["participant_id"] == patient_id].iloc[0]
                label_num = get_label(participant["Group"], participant["MMSE"])
                if label_num == -1:
                    continue
                ...
                for i in range(num_segments):
                    ...
                    if np.all(np.std(segment, axis=1) < 1e-6):
                        ...
                        continue
                    segment, flat_mask = sanitize_eeg(segment, verbose=True)
                    flat_channels_counter += flat_mask.astype(int)  # Ajouter les canaux plats par segment
                    filtered = ukf_physical_adaptive(segment, fs=fs, process_noise_init=0.1, measurement_noise_init=0.1)
                    filtered_segment = filtered.T
                    normalized_segment = normalize_eeg(filtered_segment)
                    if np.any(np.isnan(normalized_segment)) or np.any(np.isinf(normalized_segment)):
                        print(f"⚠️ Segment {i} de {eeg_path} contient NaN/inf après normalisation, ignoré")
                        continue
...
print(f"✅ Chargé {len(X_real)} segments réels (de {len(np.unique(patient_ids))} sujets)")

# Visualisation des canaux plats
plt.figure(figsize=(10, 6))
sns.barplot(x=range(num_electrodes), y=flat_channels_counter)
plt.title("Électrodes les plus souvent plates")
plt.xlabel("Numéro de l'électrode")
plt.ylabel("Nombre de détections comme plat")
plt.savefig(os.path.join(data_dir, "flat_channel_barplot.png"))
print(f"✅ Barplot des canaux plats sauvegardé sous {os.path.join(data_dir, 'flat_channel_barplot.png')}")

data_file = os.path.join(data_dir, "eeg_data.pkl")
model_file = os.path.join(data_dir, "alz_model.keras")
...
if os.path.exists(data_file):
    ...
    X = X_real
    y = y_real
    print("✅ Aucun fichier existant, on part des données réelles")
...
for i in range(num_sain):
    ...
    eeg_data, _ = sanitize_eeg(eeg_data, verbose=True)
    if np.all(np.std(eeg_data, axis=1) < 1e-6):
        print(f"⚠️ Segment simulé Sain {i} trop plat, ignoré")
        continue
    filtered = ukf_physical_adaptive(eeg_data, fs=fs, process_noise_init=0.1, measurement_noise_init=0.1)
    normed_segment = normalize_eeg(filtered.T)
    if np.any(np.isnan(normed_segment)) or np.any(np.isinf(normed_segment)):
        print(f"⚠️ Segment simulé Sain {i} contient NaN/inf après normalisation, ignoré")
        continue
...
for progression, count in class_target_counts.items():
    for i in range(count):
        ...
        eeg_data, _ = sanitize_eeg(eeg_data, verbose=True)
        if np.all(np.std(eeg_data, axis=1) < 1e-6):
            print(f"⚠️ Segment simulé {progression} {i} trop plat, ignoré")
            continue
        filtered = ukf_physical_adaptive(eeg_data, fs=fs, process_noise_init=0.1, measurement_noise_init=0.1)
        normed_segment = normalize_eeg(filtered.T)
        if np.any(np.isnan(normed_segment)) or np.any(np.isinf(normed_segment)):
            print(f"⚠️ Segment simulé {progression} {i} contient NaN/inf après normalisation, ignoré")
            continue
        X_sim.append(normed_segment)
...
def get_patient_label(pid):
    if pid.startswith("sub-"):
        ...
        return get_label(row["Group"], row["MMSE"])
    else:
        if "sain" in pid:
            return 0
        elif "début" in pid:
...
print(f"\n✅ Précision finale (par patient) : {accuracy_patient * 100:.2f}%")
...
plt.title("Matrice de confusion Alz (par patient)")
plt.colorbar()
...
plt.savefig(os.path.join(data_dir, "alz_confusion_matrix_patient.png"))
print(f"✅ Matrice de confusion sauvegardée sous {os.path.join(data_dir, 'alz_confusion_matrix_patient.png')}")

plt.figure(figsize=(10, 6))
max_probs = np.max(y_pred, axis=1)
plt.hist(max_probs, bins=20, range=(0, 1), color='skyblue', edgecolor='black')
plt.title("Distribution des probabilités maximales prédites")
plt.xlabel("Probabilité maximale")
plt.ylabel("Nombre de prédictions")
plt.grid(True, alpha=0.3)
plt.savefig(os.path.join(data_dir, "probability_histogram.png"))
print(f"✅ Histogramme des probabilités sauvegardé sous {os.path.join(data_dir, 'probability_histogram.png')}")

onnx_file = os.path.join(data_dir, "alz_model.onnx")
model_proto, _ = tf2onnx.convert.from_keras(clf, output_path=onnx_file)
print(f"✅ Modèle exporté en ONNX sous {onnx_file}")

tflite_file = os.path.join(data_dir, "alz_model.tflite")
